...
//
// Phase 5: ChatMessage is keyed by request_id (vendor_requests.id) rather
// than the legacy job_id. The seeded conversation IDs are unchanged — the
// mock jobs in mockJobs.ts kept the same UUIDs through the rename.
//
...
// "On site 35 minutes" system marker (rendered from checkin_time) reads
// correctly. All messages are within the job's created..updated window.
const marcus: ChatMessage[] = [
  {
    id: 'msg-marcus-001',
    request_id: MARCUS_ID,
    sender: 'alfred',
    content:
...
// a "Job complete" footer marker (derived from job_status === 'completed').
const hannah: ChatMessage[] = [
  {
    id: 'msg-hannah-001',
    request_id: HANNAH_ID,
    sender: 'alfred',
    content:
...
// Sarah is pending (newly dispatched) — empty thread by design. Renderer
// shows info cards + SLA banner + Accept/Reject only.
const sarah: ChatMessage[] = [];

// Daniel is completed — covers the full end-to-end composite from Figma
// 4:10457. checkin_time = 10:25Z and checkout_time = 11:00Z so the derived
// "On site 35 minutes" / "Check Out 11:00AM" markers land between the right
// messages.
const daniel: ChatMessage[] = [
  {
    id: 'msg-daniel-001',
    request_id: DANIEL_ID,
    sender: 'vendor',
